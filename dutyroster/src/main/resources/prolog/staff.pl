% 職員
staff(1, 'Alice').
staff(2, 'Bob').
staff(3, 'Charlie').
staff(4, 'David').
staff(5, 'Eve').
staff(6, 'Frank').
staff(7, 'Grace').
staff(8, 'Hannah').
staff(9, 'Ivan').
staff(10, 'Judy').

% 当番
duty(1, 'Cleaning').

% カレンダー
calendar(1, '2024-07-01', 'Monday').
calendar(2, '2024-07-02', 'Tuesday').
calendar(3, '2024-07-03', 'Wednesday').
calendar(4, '2024-07-04', 'Thursday').
calendar(5, '2024-07-05', 'Friday').
calendar(6, '2024-07-06', 'Saturday').
calendar(7, '2024-07-07', 'Sunday').
calendar(8, '2024-07-08', 'Monday').
calendar(9, '2024-07-09', 'Tuesday').
calendar(10, '2024-07-10', 'Wednesday').

% 先週分の当番割り振り(職員CD,日付,当番)
previous_duty_assignment(1, '2024-06-10', 'Cleaning').
previous_duty_assignment(2, '2024-06-12', 'Cleaning').
previous_duty_assignment(3, '2024-06-14', 'Cleaning').
previous_duty_assignment(4, '2024-06-22', 'Cleaning').
previous_duty_assignment(5, '2024-06-26', 'Cleaning').